...
After identifying and converting to a supported `Transaction` profile from an unverified source, additional consistency checks MUST be performed.
...
See [EIP assets](../assets/eip-6404/tx_hashes.py) for a definition of `compute_sig_hash` and `compute_auth_hash` that account for the various transaction types.
...
- `from`: `ExecutionSignatureV1` - An `OBJECT` containing the fields of an `ExecutionSignatureV1` structure
...
- `authority`: `ExecutionSignatureV1` - An `OBJECT` containing the fields of an `ExecutionSignatureV1` structure
